...
// var cartRouter = express.Router({ mergeParams: true }); // this is a possible alternative to make these routes connect

module.exports = {
  router,
};

// mainRouter.use('/editCart', cartRouter);
...
router.get('/addToCart', async (req, res, next) => {
  try {
    ...
    next(currentUser, cart, product, quantity);
  } catch (error) {
    console.log('an error hit in the cart capture phase');
    next(error);
  }
});
